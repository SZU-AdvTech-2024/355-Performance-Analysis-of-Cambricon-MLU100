...
#include <cstdio>
#include <iostream>
#include <thread_pool/thread_task_tracker.hpp>
...
namespace tp {
    ...
    public:
        ...
        bool post(Handler&& handler, int core_num, uint64_t cost);
    ...
    template<typename Task, template<typename> class Tracker, template<typename> class Queue>
    template <typename Handler>
    inline bool Worker<Task, Tracker, Queue>::post(Handler&& handler, int core_num, uint64_t cost)
    {
        thread_pool->addWorkload(core_num * cost);
        return m_queue.push(Tracker<Task>(std::forward<Handler>(handler), core_num, cost));
    }
        ...
        *detail::thread_id() = id;
        ...
        while (m_running_flag.load(std::memory_order_relaxed)) {
            if (m_queue.pop(tracker) || steal_donor->steal(tracker)) {
                try {
                    tracker.handler();
                    thread_pool->subWorkload(tracker.core_num * tracker.cost);
                }
                catch(...) {
